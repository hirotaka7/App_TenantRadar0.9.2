...
if zip_Capsule is not None:       
    ...
    with st.sidebar:
        ...
        def df_to_xlsx(df_Sum,df_MsbGeo,df_Stacking,df_Sansan):
            byte_xlsx=BytesIO()
            writer_xlsx=pd.ExcelWriter(byte_xlsx, engine="xlsxwriter")
            df_Sum.to_excel(writer_xlsx,index=False,sheet_name="Summary")
        ...
        st.download_button('📋 Download Excel',out_xlsx,file_name='TenantRadar.xlsx')
